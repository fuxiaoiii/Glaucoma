...
def main(path_to_config='configs/sample2.yaml', folds=5):
    ...
    test_images_dir_name = config['test_images_dir_name']
    try:
    ...
    output_dir = f"output/{exp_id}Dataaugmentation_Bicubic{model_name}"
    assert fold_num in range(5), "Fold number has to be between 0-4."
    ...
    for k in range(folds):
        ...
        # 早停设置
        early_stopping = EarlyStopping(patience=patience, verbose=True)  # count清零
        sign_earlystop = 0

        name = f"exp{exp_id}_{k}fold_{model_name}_{resize}"
        os.environ["WANDB_RUN_GROUP"] = f"exp{exp_id}_{model_name}_{resize}"
        wandb.init(project="gardnet", entity="ccnucs", name=name, reinit=True)
        ...
        csv_data = pd.read_csv(os.path.join(data_dir, "train_full.csv"))
        labels_referable = csv_data['referable']
        weight_referable = class_weight.compute_class_weight(class_weight='balanced',
                                                             classes=np.unique(labels_referable),
                                                             y=labels_referable).astype('float32')
        ...
        if model_name in timm.list_models(model_name):
            ...
            assert False, print(f"Model {model_name} not  recognized")
        ...
        with open(os.path.join(output_dir, f"log_{exp_id}_{fold_num}.csv"), "a") as f:
            ...
            if epoch_resume < epochs:
                ...
                for epoch in range(epoch_resume, epochs):
                    ...
                    for split in ['Train', 'Val']:
                        ...
                        f.write("{} Epoch {} - loss={} AUC={} F1={} Accuracy={}\n".format(split, epoch, avrg_loss, auc,
                                                                                          _f1_score, accuracy))
                        f.flush()

                        if split == 'Train':
                            wandb.log(
                                {"epoch": epoch, "train loss": avrg_loss, "train acc": accuracy, "train f1": _f1_score,
                                 "train auc": auc})
                        else:
                            wandb.log({"epoch": epoch, "val loss": avrg_loss, "val acc": accuracy, "val f1": _f1_score,
                                       "val auc": auc})
                        ...
                        if k == (folds - 1) and split == "Val":
                            ...
                            print(
                                "--------\n%s Epoch %d - mean_loss=%0.4f mean_AUC=%0.4f mean_F1=%0.4f  mean_Accuracy=%0.4f" % (
                                split, epoch, loss_mean, auc_mean, f1_mean, acc_mean))
                            f.write(
                                "--------\n{} Epoch {} - mean_loss={} mean_AUC={} mean_F1={} mean_Accuracy={}\n".format(
                                    split, epoch, loss_mean, auc_mean, f1_mean, acc_mean))
                            wandb.log({"epoch": epoch, "val mean loss": loss_mean, "val mean acc": acc_mean,
                                       "val mean f1": f1_mean, "val mean auc": auc_mean})
                            f.flush()
            ...
            if run_test:
                with open(os.path.join(output_dir, f"log_{exp_id}_{fold_num}.csv"), "a") as f:
                    ...
                    model.load_state_dict(checkpoint['state_dict'])
                    f.write("---------\nBest F1 {} for fold {}  epoch {}\n".format(checkpoint["best_f1"], str(fold_num),
                                                                                   checkpoint["epoch"]))
                    f.flush()
                    print("-----------\nBest F1 {} for fold {} epoch {}\n".format(checkpoint["best_f1"], str(fold_num),
                                                                                  checkpoint["epoch"]))
